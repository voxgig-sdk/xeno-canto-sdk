...
[MCP](https://modelcontextprotocol.io) server exposing the XenoCanto SDK as
two agent tools — `xeno-canto_list` and `xeno-canto_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/xeno-canto-mcp)
make build

# 2. Provide credentials via the environment
export XENO_CANTO_APIKEY=sk_live_xxx
...
```jsonc
// xeno-canto_list: first page of records
{ "entity": "recording" }
{ "entity": "recording", "query": { } }

// xeno-canto_load: one record by id
{ "entity": "recording", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/xeno-canto-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user xeno-canto \
     -- "$PWD"/dist/*/xeno-canto-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `xeno-canto_list` and `xeno-canto_load` tools now appear
   in new sessions. Ask the agent to *"list recording using xeno-canto"*
   and it calls `xeno-canto_list` with `{"entity":"recording"}`.
...
```sh
export XENO_CANTO_APIKEY=sk_live_xxx            # API key
export XENO_CANTO_BASE=https://api.example.com  # optional: override the API base URL
```
...
| Tool | Args | Returns |
|------|------|---------|
| `xeno-canto_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `xeno-canto_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `XENO_CANTO_APIKEY` | API key sent with every request. |
| `XENO_CANTO_BASE` | Optional override of the API base URL. |
