...
import (
	...
	omni "github.com/voxgig-sdk/xeno-canto-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.XenoCantoError); is {
				out := map[string]any{
					"name":    "XenoCantoError",
...
func omniCtx(arg any, client *sdk.XenoCantoSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.XenoCantoSDK); is {
			client = live
