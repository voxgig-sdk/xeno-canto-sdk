...
class FtCtrl extends XenoCantoControl
{
...
class FtClient
{
    ...
    private XenoCantoUtility $utility;

    public function __construct(XenoCantoUtility $utility, array $options)
    {
    ...
    public function get_utility(): XenoCantoUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (XenoCantoContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public XenoCantoContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (XenoCantoContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = XenoCantoConfig::make_feature($name);
    ...
    private static function buildUrl(XenoCantoSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof XenoCantoError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new XenoCantoResponse($fetched);
        }
    ...
    private function fail(XenoCantoContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (XenoCantoContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
