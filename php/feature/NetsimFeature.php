...
class XenoCantoNetsimFeature extends XenoCantoBaseFeature
{
    ...
    private function _simulate(XenoCantoContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
        $this->calls++;
        $call = $this->calls;
    ...
    private function _track(XenoCantoContext $ctx, array $applied): void
    {
        $client = $this->client;
