...
require_relative 'XenoCanto_types'
...
class XenoCantoSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = XenoCantoHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = XenoCantoHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, XenoCantoFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    XenoCantoUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = XenoCantoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = XenoCantoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = XenoCantoHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => XenoCantoError.new(
        "#{op}_allow",
        "XenoCantoSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = XenoCantoHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = XenoCantoError.new(
        "graphql_error", "XenoCantoSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
