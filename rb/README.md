...
raise a `XenoCantoError` (a `StandardError` subclass), so wrap
...
| `osci` | `Hash` | URLs to the three versions of oscillograms |
...
| `sono` | `Hash` | URLs to the four versions of sonograms |
...
```
rb/
├── XenoCanto_sdk.rb       -- Main SDK module
