...
boru-driven command-line client **and** interactive REPL for the XenoCanto
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/xeno-canto-cli)
make build
...
XENO_CANTO_BASE=https://api.example.com ./xeno-canto-cli list recording

# 6. No arguments -> interactive REPL
./xeno-canto-cli
xeno-canto> list recording
xeno-canto> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/xeno-canto-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/xeno-canto-cli list recording
   ```
...
Run with no arguments to open a REPL (prompt `xeno-canto>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `recording`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/xeno-canto-cli`. |
...
(`./xeno-canto-cli <expr>`) and interactively (the REPL), and expressions compose the
