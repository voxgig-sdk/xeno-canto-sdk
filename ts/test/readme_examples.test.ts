...
//     (\`new XenoCantoSDK(...)\` / \`XenoCantoSDK.test(...)\` become
//     \`XenoCantoSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"recording":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new XenoCantoSDK(...)\` and any
// \`XenoCantoSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+XenoCantoSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/XenoCantoSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
