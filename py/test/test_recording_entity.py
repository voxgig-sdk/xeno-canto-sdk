...
class TestRecordingEntity:
    ...
    def test_should_stream(self):
        ...
        base = XenoCantoSDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set XENO_CANTO_TEST_RECORDING_ENTID JSON to run live")
        client = setup["client"]
...
def _recording_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("XENO_CANTO_TEST_RECORDING_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("XENO_CANTO_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("XENO_CANTO_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("XENO_CANTO_TEST_EXPLAIN") == "TRUE",
        "live": _live,
