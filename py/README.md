...
The Python SDK for the XenoCanto API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Recording(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.XenoCanto(None).load(
    {"id": "test01"}, None
)
