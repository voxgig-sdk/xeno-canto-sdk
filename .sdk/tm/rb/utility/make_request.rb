...
module XenoCantoUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = XenoCantoResponse.new({})
    ...
    if fetch_err
      ...
      response = XenoCantoResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = XenoCantoResponse.new(fetched)
    else
