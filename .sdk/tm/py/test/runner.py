...
class XenoCantoTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            XenoCantoTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = XenoCantoTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = XenoCantoTestRunner.getenv("PROJECTENV_TEST_LIVE")
        override = XenoCantoTestRunner.getenv("PROJECTENV_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = XenoCantoTestRunner.getenv(key)
        ...
        explain = XenoCantoTestRunner.getenv("PROJECTENV_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if XenoCantoTestRunner._test_control is not None:
            return XenoCantoTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                XenoCantoTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            XenoCantoTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = XenoCantoTestRunner.load_test_control()
...
def load_env_local():
    XenoCantoTestRunner.load_env_local()


def env_override(m):
    return XenoCantoTestRunner.env_override(m)


def entity_data(v):
    return XenoCantoTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return XenoCantoTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return XenoCantoTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return XenoCantoTestRunner.load_test_control()


def live_client_options():
    return XenoCantoTestRunner.live_client_options()


def live_delay_ms():
    return XenoCantoTestRunner.live_delay_ms()
