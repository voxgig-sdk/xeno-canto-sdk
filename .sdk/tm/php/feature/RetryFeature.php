...
class XenoCantoRetryFeature extends XenoCantoBaseFeature
{
    ...
    public function init(XenoCantoContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (XenoCantoContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(XenoCantoContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(XenoCantoContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
